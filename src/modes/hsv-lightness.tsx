      ...
      () => ({
        ...
        doWrite(image: ImageData, data: Uint8Array): void {
          ...
          for (let i = 0; i < image.width * image.height; ++i) {
            ...
            let wasOneV: number
            if (previousBits) {
              wasOneV = previousBits[0]!
              previousBits = undefined
            } else {
              wasOneV = readStream.getNextBit()
            }
            v |= wasOneV << 1 // | (wasOneS << 2)
        ...
        doRead(image: ImageData): ReadResult {
          ...
          for (let i = 0; i < image.width * image.height; ++i) {
            ...
            writeStream.putBit(readMatrix[v]!)
          }
